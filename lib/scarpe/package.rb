...
module Scarpe
  ...
  class Package
    ...
    def create_appdir_tarball
      # Create self-extracting .run file (like makeself)
      run_file = File.join(@output_dir, "#{@name}-#{@arch}-linux.run")
      ...
      # Create self-extracting .run with embedded tarball
      create_self_extracting_run(run_file, tarball)

      # Cleanup intermediate tarball and AppDir
      FileUtils.rm_f(tarball)
      FileUtils.rm_rf(appdir_path)
    ...
    def create_self_extracting_run(run_file, tarball)
      # Shell script header for self-extracting archive
      header = <<~BASH
        #!/bin/bash
        # Self-extracting Scarpe application
        # Generated by scarpe package
        ...
        main() {
            ...
            EXTRACT_DIR=$(mktemp -d -t "${APP_NAME}.XXXXXX")
            trap "rm -rf '$EXTRACT_DIR'" EXIT

            echo "Extracting..."
            tail -n +$SKIP "$THIS" | tar xzf - -C "$EXTRACT_DIR"
            [ $? -ne 0 ] && { echo -e "${RED}Extraction failed!${NC}"; exit 1; }

            cd "$EXTRACT_DIR"
            if [ -d "#{File.basename(appdir_path)}" ]; then
                cd "#{File.basename(appdir_path)}"
            fi
            ...
            if [ -f "AppRun" ]; then
                ...
                echo -e "${RED}Error: Cannot find AppRun entry point${NC}"
                exit 1
            fi
        }
      ...
      # Write header + tarball
      File.open(run_file, "wb") do |f|
        f.write(header)
        File.open(tarball, "rb") do |t|
          IO.copy_stream(t, f)
        end
      end

      FileUtils.chmod(0755, run_file)
      log "   Created #{File.basename(run_file)} (self-extracting)"
    end
