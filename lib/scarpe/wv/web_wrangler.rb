    ...
    def eval_js_async(code, opts = {})
      ...
      promise = Scarpe::Promise.new(parents: (opts[:wait_for] || [])) do
        ...
        if @webview
          ...
          t_now = Time.now
          pending_evals[this_eval_serial][:wrapped_code] = wrapped_code # Hard to be sure Webview keeps a proper reference to this, so we will
          ...
          # We're mid-shutdown. No more schedulings things.
        end
      end
